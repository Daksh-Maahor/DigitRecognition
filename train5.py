...
'''w_i_h1 = np.random.uniform(-0.5, 0.5, (500, 784))
b_i_h1 = np.zeros((500, 1))
w_h1_h2 = np.random.uniform(-0.5, 0.5, (400, 500))
b_h1_h2 = np.zeros((400, 1))
w_h2_h3 = np.random.uniform(-0.5, 0.5, (300, 400))
b_h2_h3 = np.zeros((300, 1))
w_h3_h4 = np.random.uniform(-0.5, 0.5, (200, 300))
b_h3_h4 = np.zeros((200, 1))
w_h4_h5 = np.random.uniform(-0.5, 0.5, (100, 200))
b_h4_h5 = np.zeros((100, 1))
w_h5_o = np.random.uniform(-0.5, 0.5, (10, 100))
b_h5_o = np.zeros((10, 1))'''
...
for epoch in range(epochs):
    ...
    for idx in indices:
        ...
        delta_o = o - l
        w_h5_o += -learn_rate * delta_o @ np.transpose(h5)
        b_h5_o += -learn_rate * delta_o

        # back propagation : hidden5 -> hidden4
        delta_h5 = np.transpose(w_h5_o) @ delta_o * (h5 * (1-h5))
        w_h4_h5 += -learn_rate * delta_h5 @ np.transpose(h4)
        b_h4_h5 += -learn_rate * delta_h5

        # back propagation : hidden4 -> hidden3
        delta_h4 = np.transpose(w_h4_h5) @ delta_h5 * (h4 * (1-h4))
        w_h3_h4 += -learn_rate * delta_h4 @ np.transpose(h3)
        b_h3_h4 += -learn_rate * delta_h4

        # back propagation : hidden3 -> hidden2
        delta_h3 = np.transpose(w_h3_h4) @ delta_h4 * (h3 * (1-h3))
...
with open("data/neural_data5.dat", "wb") as f:
    ...
    pickle.dump(w_h3_h4, f)
    pickle.dump(b_h3_h4, f)
    pickle.dump(w_h4_h5, f)
    pickle.dump(b_h4_h5, f)
    pickle.dump(w_h5_o, f)
    pickle.dump(b_h5_o, f)
    input()
