...
from view_neural_data4 import get_neural_data

print("Running")
images, labels = get_mnist()
print("Data Acquired")

w_i_h1, b_i_h1, w_h1_h2, b_h1_h2, w_h2_h3, b_h2_h3, w_h3_h4, b_h3_h4, w_h4_o, b_h4_o = get_neural_data()
...
w_h3_h4 = np.random.uniform(-0.5, 0.5, (200, 200))
b_h3_h4 = np.zeros((200, 1))
w_h4_o = np.random.uniform(-0.5, 0.5, (10, 200))
b_h4_o = np.zeros((10, 1))'''
...
for epoch in range(epochs):
    print(f"Epoch {epoch}")
    indices = np.floor(np.random.uniform(samples_per_batch) * 60000)
    for idx in indices:
        idx = int(idx)
        img = images[idx]
        l = labels[idx]
        ...
        # forward propagation : hidden4 -> output
        o_pre = b_h4_o + w_h4_o @ h4
        o = 1 / (1 + np.exp(-o_pre))
        ...
        # back propagation : output -> hidden4
        delta_o = o - l
        w_h4_o += -learn_rate * delta_o @ np.transpose(h4)
        b_h4_o += -learn_rate * delta_o

        # back propagation : hidden4 -> hidden3
        delta_h4 = np.transpose(w_h4_o) @ delta_o * (h4 * (1-h4))
...
with open("data/neural_data4.dat", "wb") as f:
    ...
    pickle.dump(w_h4_o, f)
    pickle.dump(b_h4_o, f)
    input()
